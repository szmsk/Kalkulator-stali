...
Twoim zadaniem jest odczytanie KAŻDEJ linii tekstu (może być ich nawet około 20) i dla każdej linii rozpoznanie rodzaju elementu, jego wymiarów i ilości sztuk.

Zasady zapisu w zeszycie i rozpoznawania:
1. Sposób zapisu linii dla blachy (BLACHA):
   ...
   - "1,3,5,8,20,4" lub "6,7,8,6,4" to lista ilości sztuk po przecinku. 
     🚨 BARDZO WAŻNE: Każda z tych liczb oddzielonych przecinkiem to CAŁKOWICIE OSOBNA ilość sztuk (liczba całkowita), które należy zsumować. 
     NIGDY nie łącz ich w ułamki dziesiętne! Np. zapis "6,7,8,6,4" oznacza pięć osobnych ilości sztuk: 6, 7, 8, 6, 4. Suma to 6+7+8+6+4 = 31. Zwróć 31 jako quantity, a "6,7,8,6,4" jako rawQuantityList.

2. Sposób zapisu linii dla kształtowników standardowych (CEOWNIK lub DWUTEOWNIK):
   Przykład: "UNP 120 - 6.0 - 2,2,4" lub "IPE 160 - 12m - 1,3" lub "UPN 80 - 6m - 10" lub "UPN 120 - 6,5 - 1, 2"
   - Oznaczenie "UNP 120", "IPE 160", "UPE 140", "HEA 200" itp. to profil standardowy (isStandard = true).
     Skojarz to z odpowiednim typem: UNP/UPN/UPE to CEOWNIK, a IPE/IPN/HEA/HEB to DWUTEOWNIK.
   - Druga wartość (np. "6.0", "6,5", "12m", "6") oznacza długość odcinka w metrach (np. length = 6.0 lub 6.5 lub 12.0).
   - Trzecia wartość (np. "2,2,4" lub "1,2" lub "6,7,8,6,4") to lista ilości sztuk po przecinku. Zsumuj je jako osobne liczby całkowite.
...
3. Sposób zapisu prętów i profili zamkniętych:
   - PRĘT OKRĄGŁY GŁADKI (PRET_OKRAGLY):
     ...
     Przykład: "Ø12 - 6m - 5,5" lub "fi 16 - 3.5m - 2" lub "fi10 x 6000 - 10"
     Zwróć: detectedType = "PRET_OKRAGLY", h = średnica w mm (np. 12 lub 16), width = średnica w mm (np. 12 lub 16), isStandard = false, standardProfileName = null.
   - PRĘT KWADRATOWY (PRET_KWADRATOWY):
     Oznaczany symbolem małego kwadratu ■, "kw.", "kwadrat", "pret kw." itp.
     Przykład: "■14 - 6m - 2,3" lub "kw. 12 - 6.0 - 5" lub "15x15 pręt kw. - 3m - 2"
     Zwróć: detectedType = "PRET_KWADRATOWY", h = bok kwadratu w mm (np. 14 lub 12), width = bok kwadratu w mm (np. 14 lub 12), isStandard = false, standardProfileName = null.
   - PRĘT PŁASKI / PŁASKOWNIK (PRET_PLASKI):
     ...
     Przykład: "płaskownik 40x10 - 6m - 4,4" lub "pł. 50x8 - 6.0m - 10" lub "PL 80x10 - 6 - 5"
     Zwróć: detectedType = "PRET_PLASKI", h = grubość w mm (mniejszy wymiar, np. 10 lub 8), width = szerokość w mm (większy wymiar, np. 40, 50, 80), isStandard = false, standardProfileName = null.
   - PROFIL ZAMKNIĘTY / PROFIL BOX (PROFIL_ZAMKNIETY):
     ...
     Przykład: "profil 40x40x3 - 6m - 12,12" lub "60x40x4 - 6m - 2" lub "prof. zam. 80x80x4 - 6.0 - 5"
     Zwróć: detectedType = "PROFIL_ZAMKNIETY", h = wysokość w mm (np. 40 lub 60), width = szerokość w mm (np. 40), webThickness = grubość ścianki w mm (np. 3 lub 4), isStandard = false, standardProfileName = null.
...
5. ⚠️ WAŻNE - ROZPOZNAWANIE CYFR (POLSKIE PISMO RĘCZNE):
   ...
   - CYFRY PO PRZECINKU (UŁAMKI DZIESIĘTNE VS LISTA ILOŚCI SZTUK): Przecinek jako separator dziesiętny (np. "6,5" lub "12,2") występuje wyłącznie w sekcji WYMIARÓW (grubości, długości). Traktuj go wtedy jako jedną liczbę zmiennoprzecinkową (np. length = 6.5). Przecinek w sekcji ILOŚCI SZTUK (na samym końcu linii, np. "6,7,8,6,4") służy WYŁĄCZNIE do oddzielenia od siebie poszczególnych ilości sztuk (liczb całkowitych). Pod żadnym pozorem nie łącz ich w ułamki dziesiętne! Np. zapis "6,7,8,6,4" oznacza 6, 7, 8, 6, 4 sztuki, a nie 6.7 i 8.6! Sekcja ilości sztuk zawsze znajduje się na samym końcu linii (często po myślniku lub jako ostatnia grupa liczb) i zawiera wyłącznie liczby całkowite oddzielone przecinkami.
...
- W polu "explanation" podsumuj krótko po polsku, ile pozycji wykryłeś i jak przebiegło sumowanie sztuk.`
      };
